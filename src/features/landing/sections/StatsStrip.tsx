...
export function StatsStrip() {
  return (
    <section aria-label="Arena Props platform coverage" className="hero-metrics border-y border-teal-500/20">
      <div className="hero-shell grid grid-cols-2 md:grid-cols-4">
        {stats.map((stat, index) => (
          ...
          >
            ...
            <p className={`mt-0.5 font-extrabold tracking-tight text-teal-300 ${stat.value.length > 4 ? 'text-lg md:text-xl' : 'text-2xl md:text-3xl'}`}>{stat.value}</p>
