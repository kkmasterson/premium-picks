  ...
  return (
    <div className="rounded-2xl border border-line bg-ink-850 p-5 shadow-card sm:p-6">
      <div className="flex items-center justify-between border-b border-line pb-4">
        <div><p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-teal-300">Research filters</p><p className="mt-1 text-sm font-bold text-mist">Build the view once</p></div>
        <span className="rounded-full border border-teal-500/25 bg-teal-500/10 px-3 py-1 text-[10px] font-bold text-teal-300">38 results</span>
      </div>
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
        {filters.map(([label, value], index) => (
          <div key={label} className={`rounded-lg border p-3 ${index < 4 ? 'border-teal-500/25 bg-teal-500/[0.06]' : 'border-line bg-ink-950'}`}>
      ...
      <div className="mt-4 flex items-center justify-between rounded-lg border border-teal-500/30 bg-teal-500/10 px-4 py-3 text-xs">
        <span className="font-semibold text-teal-300">6 filters active</span><span className="text-mist-muted">Preview research state</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="rounded-2xl border border-line bg-ink-850 p-5 shadow-card sm:p-6">
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-md border border-teal-500/40 bg-teal-500/10 px-3 py-2 text-xs font-bold text-teal-300">Line 27.5</span>
      </div>
      <div className="relative mt-5 flex h-52 items-end gap-2 border-b border-line">
        <span className="absolute inset-x-0 bottom-[66%] border-t border-dashed border-teal-400/60" aria-hidden="true" />
...
function ComparisonStage() {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-line bg-ink-850 shadow-card">
      <div className="flex items-center justify-between border-b border-line px-5 py-4 sm:px-6"><div><p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-teal-300">Sample line comparison</p><p className="mt-1 text-sm font-bold text-mist">J. Brunson · Points</p></div><span className="text-[10px] text-mist-muted">Illustrative data</span></div>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[520px] text-xs">
          ...
          <tbody>{rows.map(([provider, line, over, under, label]) => (
            <tr key={provider} className={`border-b border-line/60 last:border-0 ${label ? 'bg-teal-500/[0.06]' : ''}`}><td className="px-5 py-4 font-bold text-mist">{provider}</td><td className="px-3 py-4 font-bold text-mist">{line}</td><td className="px-3 py-4 text-mist-secondary">{over}</td><td className="px-3 py-4 text-mist-secondary">{under}</td><td className="px-5 py-4 text-right">{label && <span className="rounded border border-teal-500/30 bg-teal-500/10 px-2 py-1 text-[9px] font-bold text-teal-300">{label}</span>}</td></tr>
          ))}</tbody>
        </table>
      </div>
    </div>
    ...
    <div className="rounded-2xl border border-line bg-ink-850 p-5 shadow-card sm:p-6">
      <div className="grid gap-3 sm:grid-cols-3">
        {[['BOS', '4th vs PG', '67%'], ['MIA', '11th vs PG', '50%'], ['CHI', '27th vs PG', '100%']].map(([team, rank, rate]) => (
          <div key={team} className="rounded-lg border border-line bg-ink-950 p-4"><span className="flex h-9 w-9 items-center justify-center rounded-md border border-teal-500/30 bg-teal-500/10 text-[10px] font-extrabold text-teal-300">{team}</span><p className="mt-3 text-xs font-bold text-mist">{rank}</p><p className="mt-1 text-[10px] text-mist-muted">Recent matchup hit rate</p><p className="mt-3 text-lg font-extrabold text-pos">{rate}</p></div>
        ))}
      </div>
      <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto]">
        <div className="rounded-lg border border-teal-500/30 bg-teal-500/[0.06] p-4"><p className="text-[9px] font-semibold uppercase tracking-[0.14em] text-teal-300">Research summary</p><p className="mt-2 text-sm font-semibold leading-relaxed text-mist">Market, recent form, available lines, and matchup context stay connected.</p></div>
            ...
            <article key={s.n} className="relative grid items-center gap-8 lg:grid-cols-2 lg:gap-20">
              ...
              <Reveal direction={i % 2 === 0 ? 'left' : 'right'} className={`min-w-0 ${i % 2 === 1 ? 'lg:order-2' : ''}`}>
                <div className={i % 2 === 1 ? 'lg:pl-4' : 'lg:pr-4'}>
                  ...
                  <p className="mt-5 flex items-start gap-2.5 text-sm font-semibold leading-relaxed text-mist-secondary"><span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-teal-400 shadow-[0_0_12px_rgba(45,212,191,0.55)]" />{s.detail}</p>
                </div>
              </Reveal>
              <Reveal
