    ...
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label={`${player.name} research`}>
      ...
      <div className="absolute inset-y-0 right-0 flex w-full max-w-md flex-col border-l border-[#222] bg-[#0e0e0e] shadow-2xl">
        <div className="flex items-start gap-3 border-b border-[#1c1c1c] p-4">
          ...
          <button onClick={closeDrawer} aria-label="Close research drawer" className="rounded p-1.5 text-zinc-400 hover:bg-[#1a1a1a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-4">
          <section aria-label="Current prop" className="rounded-lg border border-[#222] bg-[#121212] p-3">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-bold text-teal-300">{best.book} {formatOdds(best.over)}</p>
              </div>
            </div>
          </section>
          ...
          <section aria-label="Projection" className="rounded-lg border border-teal-500/25 bg-teal-500/[0.05] p-3">
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-teal-300">Arena Props Projection</p>
            <div className="flex items-end justify-between">
              <div>
                <p className="text-2xl font-bold tabular-nums text-teal-300">{prop.projection.toFixed(1)}</p>
          ...
          <section aria-label="Sportsbook lines" className="rounded-lg border border-[#222] bg-[#121212]">
            ...
            <div className="divide-y divide-[#181818]">
              {prop.books.map((b) => {
                ...
                return (
                  <div key={b.book} className={cn('flex items-center gap-2 px-3 py-2 text-xs tabular-nums', isBest && 'bg-teal-500/[0.05]')}>
                    <span className={cn('w-9 font-bold', isBest ? 'text-teal-300' : 'text-zinc-300')}>{b.book}</span>
                    ...
                    {isBest && <span className="ml-auto rounded bg-teal-400 px-1 py-px text-[9px] font-bold text-black">BEST PRICE</span>}
                  </div>
                );
              })}
            </div>
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] gap-2 border-t border-[#1c1c1c] p-4 max-[430px]:grid-cols-2">
          <button
            ...
            className="flex min-w-0 items-center justify-center gap-1.5 rounded-md bg-teal-500 py-2.5 text-sm font-bold text-black hover:bg-teal-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-300 max-[430px]:col-span-2"
          >
          ...
          <button
            ...
            className={cn(
              'flex items-center justify-center gap-1.5 rounded-md border px-4 py-2.5 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500',
              isSaved ? 'border-teal-500/50 bg-teal-500/10 text-teal-300' : 'border-[#2a2a2a] text-zinc-300 hover:bg-[#181818]',
            )}
          >
          ...
          <button
            ...
            className={cn('flex items-center justify-center gap-1.5 rounded-md border px-3 py-2.5 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500', inBuilder ? 'border-teal-500/50 bg-teal-500/10 text-teal-300' : 'border-[#2a2a2a] text-zinc-300 hover:bg-[#181818]')}
          >
