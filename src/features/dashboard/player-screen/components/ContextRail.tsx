    ...
    <section className="rounded-xl border border-[#202020] bg-[#101010]">
      ...
      <div className="p-3">
        {tab === 'movement' ? (
          ...
          <div><div className="mb-3 flex items-center justify-between"><div><p className="text-[9px] uppercase tracking-wider text-zinc-600">Current / filtered average</p><p className="text-lg font-bold text-white">{line} <span className="text-xs font-normal text-zinc-500">/ {market.average?.toFixed(1) ?? '—'}</span></p></div><span className="rounded bg-teal-500/10 px-2 py-1 text-xs font-semibold text-teal-300">{market.hitRates.l10 ?? '—'}% L10</span></div><div className="no-scrollbar flex gap-2 overflow-x-auto">{market.history.filter((entry) => entry.availability === 'played').slice(0, 5).map((entry) => { const over = (entry.value ?? 0) > (entry.line ?? 0); return <div key={entry.id} className={cn('min-w-[118px] shrink-0 rounded-lg border p-2.5', over ? 'border-emerald-500/25 bg-emerald-500/[0.04]' : 'border-red-500/25 bg-red-500/[0.04]')}><div className="flex justify-between text-[9px]"><span className="text-zinc-500">{entry.date}</span><span className={over ? 'font-bold text-emerald-400' : 'font-bold text-red-400'}>{over ? 'OVER' : 'UNDER'}</span></div><p className="mt-2 text-xs text-zinc-300">{entry.home ? 'vs' : '@'} {entry.opponent}</p><p className="mt-1 text-[10px] text-zinc-500">Line {entry.line} · Result <span className="font-bold text-zinc-200">{entry.value}</span></p></div>; })}</div>{!market.history.length && <p className="py-6 text-center text-xs text-zinc-500">No prop history matches the selected filters.</p>}</div>
        )}
      </div>
    </section>
  );
