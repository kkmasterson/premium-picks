...
export const mockPlayerResearchAdapter: PlayerResearchAdapter = {
  getPlayerResearch(player, profile): PlayerResearchViewModel {
    ...
    return {
      ...
      lineMovement: Array.from({ length: 6 }, (_, index) => ({ id: `${player.id}-movement-${index}`, line: round(defaultMarket.canonicalLine + (index - 2) * defaultMarket.definition.step, defaultMarket.definition.step), provider: defaultMarket.offers[index % Math.max(1, defaultMarket.offers.length)]?.shortName ?? 'PP', secondsAgo: 45 + index * 240, direction: index === 0 ? 'up' : index === 1 ? 'down' : 'flat', delta: index === 0 ? defaultMarket.definition.step : index === 1 ? -defaultMarket.definition.step : 0 })),
